...
\newcommand{\studentname}{Lucas Lamka}
...
Three bodies of research inform the design of \textit{Latin Explorer}'s
applets. The first is Stephen Krashen's Input Hypothesis, which holds that
language acquisition occurs when learners encounter material just beyond
their current level of competence---what Krashen calls $i + 1$
\autocite{Krashen1982}. For a Latin student reading Virgil, this means
structured exposure to difficult text paired with targeted support precisely
where comprehension breaks down.

The second framework is Lev Vygotsky's concept of the Zone of Proximal
Development (ZPD): the space between what a learner can accomplish
independently and what becomes possible with guidance from a more capable
other \autocite{Vygotsky1978}. Krashen and Vygotsky together describe both
\textit{what} students should encounter and \textit{how} the teacher should
position herself to help. The teacher who can see, at a glance, which lines
of the \textit{Georgics} are giving students trouble is a teacher who can
intervene inside the ZPD rather than guessing at it.

The third framework is Universal Design for Learning (UDL), which calls for
multiple means of representation, engagement, and expression so that all
learners can access content \autocite{CAST2018}. Taken together, the three
applets embody each of these principles: the \textit{Georgics} applet
scaffolds reading and comprehension (Krashen + Vygotsky); the Etymology
applet activates students' existing linguistic knowledge as a bridge to
Latin vocabulary (UDL representation); and the Graffiti applet invites
productive written output in Latin (UDL expression). The framework, in a
single sentence: \textit{the applets provide comprehensible input through
scaffolded interaction across multiple modalities}.
...
where in the text students are struggling---making the Zone of Proximal
Development visible rather than a matter of guesswork \autocite{Vygotsky1978}.
Reading ancient texts with attention to language and meaning is at the heart
of what scholars call ``deep reading'' \autocite{Wolf2018}, and this applet
is designed to support rather than shortcut that process.

%% ------------------------------------------------------------
\subsection{A Living Language: Graffiti and Etymology}
%% ------------------------------------------------------------

The Etymology applet connects students' existing knowledge of English to
...
the subject. In UDL terms, activating prior linguistic knowledge is a means
of \textit{representation}---meeting students where they already are before
asking them to go somewhere new \autocite{CAST2018}. With this applet,
students are encouraged to search for derivatives in whatever languages they
speak, connecting the unit topic to their own linguistic
experience---an approach consistent with heritage-language pedagogy
\autocite{Polinsky2018}.

The Graffiti applet addresses the third UDL principle: \textit{expression}.
The ability to write Latin is often not taught in the classroom, yet
...
\autocite{SharwoodSmith1993}. Topics such as colors, feelings, and weather
allow students to exercise vocabulary while tracking their own progress
since the start of the year. Prompts related to a unit topic are provided
to facilitate mastery of sentence frames and vocabulary.

%% ============================================================
\section{Why Not AI in the Applets?}
%% ============================================================

\textit{Latin Explorer} was built \textit{with} AI---agentic coding tools
made the site possible---but the applets themselves ask nothing of an
algorithm. When a student types a question into the \textit{Georgics}
applet, no model answers it. When a student submits a translation, no model
checks it. The teacher does those things.

This is a deliberate pedagogical choice, not a technical limitation.
Translation is not a retrieval task. It is an interpretive act. The choices
a student faces---which English word best captures \textit{ferrum}, how to
handle an ablative absolute, whether Virgil is being ironic---are exactly
where Latin learning happens. An AI that resolved those choices instantly
would not be scaffolding the student's Zone of Proximal Development; it
would be collapsing it \autocite{Vygotsky1978}. The struggle \textit{is}
the comprehensible input \autocite{Krashen1982}.

The teacher's role in this model is irreplaceable. Knowing at a glance
that an entire class is stuck on the same line of Virgil is not information
an AI needs---it is information a \textit{teacher} needs, so that she can
walk into the next class knowing exactly where to intervene. The applets
are designed to make the teacher more effective, not to substitute for her.

\textcite{Colace2025} describe this balance as the ``reflective use of
assisted translation'': AI as an analytical scaffold, never as a replacement
for the student's own hermeneutic effort. \textit{Latin Explorer} holds to
the same line. There is, after all, no royal road.
